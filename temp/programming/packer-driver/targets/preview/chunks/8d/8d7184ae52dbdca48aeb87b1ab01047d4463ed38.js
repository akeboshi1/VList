    ...
    execute: function () {
      ...
      _export("VList", VList = (_dec = ccclass('VList'), _dec2 = property({
        ...
        set scrollDir(val) {
          ...
          var scrollView = scrollNode.getComponent(_crd && FixedScrollView === void 0 ? (_reportPossibleCrUseOfFixedScrollView({
            error: Error()
        ...
        get overflowDir() {
          var hor = this.strech_hor == EStrechType.Overflow ? 1 : 0;
          var ver = this.strech_ver == EStrechType.Overflow ? 2 : 0;
          return hor + ver;
        }
        ...
        onChangeParams() {
          ...
          if (isInEditorMode) {
            ...
            for (var i = 0; i < this.preItemNum; i++) {
              var idx = this.isRevertSiblingOrder ? this.preItemNum - i - 1 : i;
            ...
            var childrenTmp = [...this.content.children];

            for (var _i = this.preItemNum; _i < childrenTmp.length; _i++) {
              childrenTmp[_i].destroy();
            }
        ...
        get progress() {
          ...
          var validOffset = v2(this.viewSize.x / 2, -this.viewSize.y / 2).add(this.contentOffset);
          var validSize = this.isLoop ? v2(info.boundSize.x + info.spaceX, info.boundSize.y + info.spaceY) : v2(info.size.x - this.viewSize.x, info.size.y - this.viewSize.y);
          var x = -validOffset.x / validSize.x;
          var y = validOffset.y / validSize.y;
        ...
        set progress(val) {
          ...
          var info = this.layoutInfo;
          var com = this.scrollRect.getComponent(_crd && FixedScrollView === void 0 ? (_reportPossibleCrUseOfFixedScrollView({
            error: Error()
          }), FixedScrollView) : FixedScrollView);
          com.stopAutoScroll();
          var validSize = this.isLoop ? v2(info.boundSize.x + info.spaceX, info.boundSize.y + info.spaceY) : v2(info.size.x - this.viewSize.x, info.size.y - this.viewSize.y);
          var offsetProcess = validSize;
          ...
          var validOffset = v3(-offsetProcess.x - this.viewSize.x / 2, offsetProcess.y + this.viewSize.y / 2, 0);
        ...
        refreshItemCom() {
          ...
          var scan = function scan(n, path) {
            ...
            if (!subList) for (var i = 0; i < n.children.length; i++) {
              ...
              if (c.name.startsWith(_this.comPrefix)) {
                var renderCom = c.getComponent(UIRenderer);
                if (renderCom) _this.comShowList.push("" + renderCom.name);else _this.comShowList.push(c.name + "<Node>");

                _this.comKeyList.push(c.name);

                _this.comPathList.push("" + path + c.name);
              }
          ...
          var _loop = function _loop() {
            ...
            _this.comShowList.push(...list.comShowList.map(e => list.node.name + " / " + e));
          };
        ...
        onScrolling() {
          ...
          for (var i = 0; i < this.relativeListCom.length; i++) {
            var target = this.relativeListCom[i];
        ...
        init(itemCallback) {
          ...
          if (this.isRenderByLayer && this.itemPrefab) {
            ...
            var scan = function scan(n, path, position) {
              if (path === void 0) {
                path = "";
              }
              ...
              node.setParent(_this2.content);
              trans.anchorPoint = v2(0, 1);
              node.position = position;

              _this2.layerMap.set(curPath, node);

              if (!n.getComponent(VList)) if (!_this2.ignoreComList.some(m => n.getComponent(m))) for (var i = 0; i < n.children.length; i++) {
                var c = n.children[i];
        ...
        getInfoByData(data, idx) {
          ...
          info.call = function (cb, realIdx) {
            if (realIdx === void 0) {
              realIdx = "all";
            }

            if (realIdx == "all") info.renderItems.forEach(e => cb(info, e));else {
              var item = info.renderItems.find(e => e.realIdx == realIdx);
        ...
        setData(datas, parentInfo, ignoreReset) {
        ...
        addData(data, insertIdx) {
          ...
          var exeFunc = () => {
            ...
            if (insertIdx == "none" || insertIdx >= cnt) {
              var newInfo = this.getInfoByData(data, cnt);
              this.infos.push(newInfo);
            } else {
              if (insertIdx < 0) insertIdx = 0;

              var _newInfo = this.getInfoByData(data, insertIdx);

              var after = [...this.infos].slice(insertIdx);
              var before = [...this.infos].slice(0, insertIdx);
              this._infos = before.concat(_newInfo).concat(after);
            }
        ...
        deleteData(data) {
          var idx = this.infos.findIndex(e => e.data == data);
        ...
        refreshItem(infoIdx, realIdx) {
          ...
          if (realIdx == "all") {
            ...
            var _renderItem = info.renderItems.find(e => e.realIdx == realIdx);

            if (_renderItem) {
              info.node = _renderItem.node;
              this.cb.onData && this.cb.onData(info, _renderItem);
            }
          }
        }
        ...
        unregister(key, nodeEvent) {
          var uniKey = "this/" + key + "_" + nodeEvent;
          var index = this.registerInfos.findIndex(e => e.uniKey == uniKey);
          if (index < 0) return;
        ...
        registerNodeEvt(info, renderItemInfo, registerInfo) {
          var node = registerInfo.key == "" ? renderItemInfo.node : renderItemInfo.getNode(registerInfo.key);

          var cb = node["custom_event_" + registerInfo.uniKey] = () => registerInfo.callback(info);
        ...
        getChildByPath_native(node, path, hasHead) {
          if (hasHead === void 0) {
            hasHead = false;
          }
          ...
          if (!this.isRenderByLayer) return node.getChildByPath(path);else {
            ...
            var childInfo = children.find(e => e.path == (hasHead ? path : this.rootLayer.name + "/" + path));
        ...
        getItemComByKey(node, key, comType) {
          if (!node) return null;
          var pathIdx = this.comKeyList.findIndex(e => e == key);
          ...
          var targetNode = this.getChildByPath_native(node, path);
        ...
        refreshNodeTrans(info) {
          ...
          var _loop2 = function _loop2() {
            var item = info.renderItems[i];
            var pos = V3(_this3.getPosInfo(item.realIdx).center);

            if (!_this3.isRenderByLayer) {
              item.node.position = pos;
              var wid = item.node.getComponent(Widget);
              ...
              item.node.getComponent(UITransform).setContentSize(_this3.realItemSize);

              var children = _this3.itemChildMap.get(item.node);

              children.forEach(e => {
                if (e.skipParent) return;
                var widget = e.child.getComponent(Widget);

                if (e.child != item.node && widget) {
                  var parentTrans = _this3.getChildByPath_native(item.node, getParentPath(e.path), true).getComponent(UITransform);

                  var childTrans = e.child.getComponent(UITransform);
                  var res = (_crd && WidgetUtils === void 0 ? (_reportPossibleCrUseOfWidgetUtils({
                    error: Error()
                  }), WidgetUtils) : WidgetUtils).align(parentTrans, e.child.getComponent(UITransform));
                  childTrans.setContentSize(res.size);
                }

                e.child.setParent(_this3.layerMap.get(e.path.replace(/\//g, "-")));
                e.child.position = pos;
              });
            }
          };

          for (var i = 0; i < info.renderItems.length; i++) {
            _loop2();
          }
        }
        ...
        getNode(info) {
          ...
          if (!this.isRenderByLayer) {
            ...
            if (this.nodePools.length > 0) {
              res = this.nodePools.pop();
              var children = this.itemChildMap.get(res);
              children.forEach(e => {
                if (e.skipParent) return;
              ...
              var scan = function scan(n, path, isSkipParent) {
                ...
                var childSkipParent = isSkipParent || _this4.ignoreComList.some(m => n.getComponent(m));
              ...
              this.refreshDelayFuncs.push(() => this.cb.onInstantiate && this.cb.onInstantiate(_initInfo));
            }
          }
        ...
        crd2idx(crd) {
          var c = this.isLoop ? crd.x : clamp(crd.x, 0, this.realCol - 1);
          var r = this.isLoop ? crd.y : clamp(crd.y, 0, this.realRow - 1);
        ...
        progress2idx(progress) {
          ...
          var lp = v2();
          var loopSize = v2(this.layoutInfo.boundSize.x + this.realSpaceX, this.layoutInfo.boundSize.y + this.realSpaceY);
        ...
        idx2progress(realIdx) {
          if (this.listType == EListType.Layout || this.scrollDir == EScrollDir.Both) return -1;
          var lp = this.getPosInfo(realIdx).center;
          var contentSize = v2(this.layoutInfo.boundSize.x + this.realSpaceX, this.layoutInfo.boundSize.y + this.realSpaceY);
        ...
        getVec() {
          ...
          var unitSize = v2(this.realItemSize.x + layout.spaceX, this.realItemSize.y + layout.spaceY);
        ...
        focus(target, time, endCallback) {
          ...
          var scrollView = this.scrollRect.getComponent(_crd && FixedScrollView === void 0 ? (_reportPossibleCrUseOfFixedScrollView({
            error: Error()
          ...
          switch (target) {
            ...
            default:
              ...
              var anchorOffset = v2((pos.x - containerSize.width / 2) / scrollSize.x, (pos.y + containerSize.height / 2) / -scrollSize.y);
        ...
        recycleRenderItem(info, renderItem) {
          ...
          var targetNode = renderItem.node;
        ...
        refreshStruct() {
          ...
          switch (this.listType) {
            ...
            case EListType.Page:
              ...
              var itemRectTrans = itemRect.getComponent(UITransform);
        ...
        alignContentPos(reset) {
          ...
          var contentSize = this.layoutInfo.size;
          var itemSize = this.realItemSize;
          var contentPos = this.contentOffset;
        ...
        updateLayout(len) {
          ...
          var getCol = null;
          var getRow = null;
          var cnt = 0; //0:horizontal  1:vertical
        ...
        testItemIdxByLp(lp) {
          ...
          var unitSize = v2(this.realItemSize.x + this.realSpaceX, this.realItemSize.y + this.realSpaceY);
          var vec = Vec2.divide(unitSize, lp, unitSize);
          vec.y = -vec.y;
          var crd = Vec2.floor(vec, vec);
        ...
        testVisibleByIdx(realIdx) {
          ...
          var accuracy = 1;
          var viewSize = this.trans.contentSize;
          var posInfo = this.getPosInfo(realIdx);
          var minViewPos = v3(accuracy, -viewSize.height + accuracy, 0);
          var maxViewPos = v3(viewSize.width - accuracy, -accuracy, 0);
          var itemMinPos = v3(posInfo.xMin + this.contentOffset.x + viewSize.width / 2, posInfo.yMin + this.contentOffset.y - viewSize.height / 2, 0);
          var itemMaxPos = v3(posInfo.xMax + this.contentOffset.x + viewSize.width / 2, posInfo.yMax + this.contentOffset.y - viewSize.height / 2, 0);
        ...
        getPosInfo(realIdx) {
          ...
          var horLayout = this.layoutDir == EDir.Horizontal;
          var loopIdx = Math.floor(realIdx / info.num);
          var verScrollLimit = !(this.scrollDir & EScrollDir.Vertical);
          var horScrollLimit = !(this.scrollDir & EScrollDir.Horizontal);
        ...
        getRealIdxRangeInView() {
          var accuracy = 1;
          var startIdx = this.testItemIdxByLp(v2(-this.contentOffset.x + accuracy, -this.contentOffset.y - accuracy));
          var endIdx = this.testItemIdxByLp(v2(this.viewSize.x - this.contentOffset.x - accuracy, -this.viewSize.y - this.contentOffset.y + accuracy));
          return {
            startIdx,
            endIdx
          };
        ...
        refreshView(force) {
          ...
          var idxRange = this.getRealIdxRangeInView();
          var startIdx = idxRange.startIdx;
          var endIdx = idxRange.endIdx;
          var renderMaps = new Map();
          var renderVisitMap = new Map(); //根据视口包含的可能索引算出所有在视口内的和在视口外的

          var _loop3 = function _loop3(i) {
            var infoIdx = _this5.positiveMod(i, _this5.infos.length);

            var renderInfos = renderMaps.get(infoIdx);
            ...
            var isVisible = _this5.testVisibleByIdx(i);

            var info = _this5.infos[infoIdx];
            var curItem = info.renderItems.find(e => e.realIdx == i);
            ...
            } else if (!curItem && isVisible) renderInfos.enterInfos.push(_this5.getRenderItemInfo(i, _this5.getNode(info), info));
          };
          ...
          for (var _i2 = 0; _i2 < this.infos.length; _i2++) {
            if (!this.infos[_i2].isVisible) continue;
            var _info = this.infos[_i2];

            if (renderVisitMap.has(_i2)) {
              var visitMap = renderVisitMap.get(_i2);

              for (var j = 0; j < _info.renderItems.length; j++) {
                if (!visitMap.has(_info.renderItems[j].realIdx)) renderMaps.get(_i2).exitInfos.push(_info.renderItems[j]);
              }
            } else {
              renderMaps.set(_i2, {
                stayInfos: [],
                enterInfos: [],
                exitInfos: _info.renderItems
              });
            }
          }
