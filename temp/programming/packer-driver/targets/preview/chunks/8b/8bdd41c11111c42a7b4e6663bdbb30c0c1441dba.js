    ...
    execute: function () {
      ...
      _export("DynamicList", DynamicList = (_dec = ccclass('DynamicList'), _dec2 = property(Prefab), _dec3 = property(Node), _dec4 = property(Node), _dec5 = property(Node), _dec6 = property({
        ...
        setData(args) {
          ...
          this.renderInfos = [];
          var totalHeight = this.paddingTop;
        ...
        refreshView() {
          var contentPos = this.content.position;
          var viewSize = this.view.getComponent(UITransform).contentSize;
          if (this.renderInfos.length === 0) return;
          var viewTopY = this.view.position.y + viewSize.height / 2;
          var viewBottomY = this.view.position.y - viewSize.height / 2;
          var currentY = -this.paddingTop;

          for (var i = 0; i < this.renderInfos.length; i++) {
            var info = this.renderInfos[i];
            var itemHeight = info.height;
            var item = info.item;
            var dirty = false;
            var itemTopY = currentY + contentPos.y;
            var itemBottomY = currentY - itemHeight + contentPos.y;
            var isInView = itemBottomY < viewTopY + this.space && itemTopY > viewBottomY - this.space;
